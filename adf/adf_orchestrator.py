...
from .utils import make_adf_id, setup_logger
from .ci_adapter import get_ci_adapter, log_group
...
def check_env_vars(logger: Optional[logging.Logger] = None) -> None:
    ...
    if missing_vars:
        ...
        # Also log to CI system
        if ci.is_ci():
            ci.log_error(f"Missing required environment variables: {', '.join(missing_vars)}")
...
def parse_args(logger: Optional[logging.Logger] = None) -> Tuple[str, Optional[str]]:
    ...
    adf_id = sys.argv[2] if len(sys.argv) > 2 else None

    return issue_number, adf_id


def format_issue_message(
    adf_id: str, agent_name: str, message: str, session_id: Optional[str] = None
) -> str:
    """Format a message for issue comments with ADF tracking."""
    if session_id:
        return f"{adf_id}_{agent_name}_{session_id}: {message}"
    return f"{adf_id}_{agent_name}: {message}"
...
def build_plan(
    issue: GitHubIssue, command: str, adf_id: str, logger: logging.Logger
) -> AgentPromptResponse:
...
def get_plan_file(
    plan_output: str, adf_id: str, logger: logging.Logger
...
def implement_plan(
    plan_file: str, adf_id: str, logger: logging.Logger
) -> AgentPromptResponse:
...
def git_branch(
    ...
    request = AgentTemplateRequest(
        ...
        args=[issue_type, adf_id, str(issue.number), f'"{issue.title}"'],
        adf_id=adf_id,
        model="sonnet",
    )
...
def pull_request(
    ...
    request = AgentTemplateRequest(
        ...
        args=[branch_name, issue.model_dump_json(by_alias=True), plan_file, adf_id],
        adf_id=adf_id,
        model="sonnet",
    )
...
def main():
    ...
    issue_number, adf_id = parse_args()

    # Generate ADF ID if not provided
    if not adf_id:
        adf_id = make_adf_id()
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(adf_id, "ops", f"✅ ADF workflow completed successfully"),
    )
